...
trait UploadAble
{
    ...
    public function uploadOne(UploadedFile $file, $folder = null, $fileName = null, $storageDisk = 'public', array $resizeFolder = [])
    {
        try {
            ...
            // ✅ Laravel 9+ compatible directory creation
            Storage::disk($storageDisk)->makeDirectory($folder);
            ...
            if ($isOrignalFileUploaded) {
                if (!empty($resizeFolder)) {
                    foreach ($resizeFolder as $resizeType) {
                        if (in_array(strtolower($fileExtension), ['mp4', 'mp3', 'wav', 'm4a', 'aac', 'ogg', 'webm', 'mpeg', 'avi', 'mov'])) {
                            $isFileProcessed = true; // $this->makeVideoTrailerAndSave($storageDisk, $fileName, $resizeType);
                            if (!$isFileProcessed) {
                                return false;
                            }
                        } else if (in_array(strtolower($fileExtension), ['jpg', 'jpeg', 'png', 'gif', 'svg', 'webp', 'bmp'])) {
                            $isFileProcessed = $this->imageResizeAndSave(
                                Storage::disk($storageDisk)->path($folder . '/' . $fileName),
                                $fileName,
                                $fileVariations[$resizeType],
                                $storageDisk
                            );
                            if (!$isFileProcessed) {
                                return false;
            ...
            \Log::error('uploadOne failed: ' . $e->getMessage());
            return false;
        }
    }
    ...
    private function imageResizeAndSave($file, $fileName, $fileVariations, $storageDisk)
    {
        try {
            $image = Image::make($file)
                ->resize(
                    $fileVariations['size'][0],
                    $fileVariations['size'][1],
                    function ($constraint) {
    ...
    public function getAudioDuration(UploadedFile $file): ?float
    {
        try {
            $getID3 = new \getID3();
            $fileInfo = $getID3->analyze($file->getPathname());

            // Returns duration in seconds (e.g. 213.45)
            return isset($fileInfo['playtime_seconds']) ? round($fileInfo['playtime_seconds'], 2) : null;
        } catch (\Exception $e) {
            logger('getAudioDuration failed: ' . $e->getMessage());
            return null;
        }
    }
}

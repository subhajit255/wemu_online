                                ...
                                <div class="d-flex align-items-center gap-2 gap-lg-3">
                                    <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                                        <button type="button" class="btn btn-dark goTo"
                                            data-action="{{ route('admin.coupon.add') }}">Add
                                            Coupon</button>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <tbody class="fw-semibold text-gray-600">
                                                @forelse ($details as $detail)
                                                    <tr>
                                                        ...
                                                        {{-- <td>{{ $detail->max_user ?? 'N/A' }}</td> --}}

                                                        <td>{{ \Carbon\Carbon::parse($detail->start_date)->format('jS F Y') }}
                                                        </td>
                                                        <td>{{ \Carbon\Carbon::parse($detail->end_date)->format('jS F Y') }}
                                                        </td>

                                                        <td>{{ $detail->used ?? 0 }} Coupon(s) Used</td>
                                                        <td>
                                                            <div class="form-check form-switch">
                                                                <input type="checkbox" data-uuid="{{ $detail->uuid }}"
                                                                    data-table="coupons" class="form-check-input isVerified"
                                                            ...
                                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                                ...
                                                                <div class="menu-item px-3">
                                                                    <a class="menu-link px-3"
                                                                        href="{{ route('admin.coupon.add', $detail->uuid) }}">Edit</a>
                                                                </div>
                                                                <div class="menu-item px-3">
                                                                    <a href="javascript:void(0)" data-table="coupons"
